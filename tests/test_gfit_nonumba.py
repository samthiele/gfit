...
# Simulate missing numba before gfit is imported (or re-imported).
os.environ["GFIT_NO_NUMBA"] = "1"

for _name in list(sys.modules):
    if _name == "gfit" or _name.startswith("gfit."):
        del sys.modules[_name]

import numpy as np
from gfit import initialise, evaluate, gfit
from gfit._accel import HAS_NUMBA
from gfit.internal import fit_mgauss, fit_amgauss, amgauss, mgauss
from gfit.util import split_coeff, stack_coeff, rand_signal


def setUpModule():
    assert not HAS_NUMBA, "nonumba tests require HAS_NUMBA to be False"


class MyTestCaseNoNumba(unittest.TestCase):

    def test_00_no_numba_backend(self):
    ...
    def test_05_gfit_single_sym(self):
        ...
        x0 = initialise(x, X, 3, sym=True, d=4)  # compute initial values
        gfit(x, X, x0, 3, sym=True, nthreads=1, vb=True) # run optimisation
    ...
    def test_06_gfit_single_asym(self):
        ...
        x0 = initialise(x, X, 3, sym=False, d=4)  # compute initial values
        gfit(x, X, x0, 3, sym=False, nthreads=1, vb=True) # run optimisation
    ...
    def test_09_hull(self):
        ...
        remove_hull(np.array([y]), upper=True, div=True)

    def test_10_benchmark(self):
