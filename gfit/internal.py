...
def refine(x, X, x0, n=3, vb=False):
    """
...
# @jit(nopython=True, parallel=True)
# def refine(x, X, x0, n=5, nthreads=-1):
#     """
#     Use a best-fit quadratic parabola to refine estimates of absorption feature position (as returned by init(...).
#     N.B. this is much faster than gaussian fitting!
#
...
#     # init output
#     x1 = x0.copy()
#     for i in prange(0, X.shape[0]):  # loop through signals
#         # loop through n maxima
#         for j in range(1, x0.shape[-1], 4):
#             # get fitting interval
#             b = int(np.argmin(np.abs(x0[i, j] - x)))
#             b0 = max(b - n, 0)
#             b1 = min(b + n, x.shape[0] - 1)
#
#             # do polyfit
#             _a, _b, _c = np.polyfit(x[b0:b1], X[i, b0:b1], deg=2)
#             # print(_a,_b,_c)
#
#             if _a > 0:  # not a valid maxima
#                 x1[i, j - 1] = 0  # set height to zero
#             else:
#                 x1[i, j] = -_b / (2 * _a)  # refine position
#                 x1[i, j - 1] = _c - (_b ** 2 / (4 * _a))  # refine height
#
#     # reset default nthreads
#     if nthreads != -1:
#         numba.set_num_threads(t)
